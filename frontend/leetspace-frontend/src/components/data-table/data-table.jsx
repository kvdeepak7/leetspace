...
export function DataTable({ data, columns, onDataChange }) {
  ...
  const handleMassDelete = async () => {
    ...
    try {
      ...
      setRowSelection({});
      toast.success(
        `Successfully deleted ${selectedCount} problem${selectedCount > 1 ? "s" : ""}!`,
        {
          style: {
      ...
      if (onDataChange) {
        const updatedData = data.filter(
          (problem) => !selectedIds.includes(problem.id)
        );
    ...
    <div>
      <div className="py-4 border-b border-gray-200 dark:border-zinc-700 space-y-3">
        ...
        <div className="hidden sm:flex items-center gap-3">
          ...
          <div className="flex items-center gap-4 shrink-0">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <span
                  className={`${revisitOnly ? "translate-x-6" : "translate-x-1"} inline-block h-4 w-4 transform rounded-full bg-white shadow transition`}
                />
              </button>
            </div>
          </div>
        ...
        <div className="flex items-center gap-4 sm:hidden">
          <div className="flex items-center gap-2">
            <Checkbox checked={selectionMode} onCheckedChange={setSelectionMode} />
            <span className="text-sm text-gray-700 dark:text-gray-300">Select</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-sm text-gray-700 dark:text-gray-300">Revisit</span>
              ...
              table.getRowModel().rows.map((row) => (
                ...
                >
                  {row.getVisibleCells().map((cell) => (
                    <TableCell
                      key={cell.id}
                      className={cn("px-4 py-2", cell.column.columnDef.meta?.cellClass)}
                    >
                      {flexRender(
